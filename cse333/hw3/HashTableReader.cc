...
list<IndexFileOffset_t> HashTableReader::LookupElementPositions(
  ...
  ElementPositionRecord pos_rec;
  IndexFileOffset_t rec_pos = bucket_rec.position;
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    Verify333(fseek(file_, rec_pos, SEEK_SET) == 0);
    Verify333(fread(&pos_rec, sizeof(ElementPositionRecord), 1, file_) == 1);
    pos_rec.ToHostFormat();
    ret_val.push_back(pos_rec.position);
    rec_pos += sizeof(ElementPositionRecord);
  }
