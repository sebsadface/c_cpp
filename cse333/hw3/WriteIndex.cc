...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  for (int i = 0; i < doctable_bytes + memidx_bytes; i++) {
    if (fread(&nextbyte, sizeof(uint8_t), 1, f) != 1) {
      return kFailedWrite;
    }

    crc.FoldByteIntoCRC(nextbyte);
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    if (chain_len != 0) {
      int bytes_written = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
      if (bytes_written < 0) {
        return kFailedWrite;
      }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    element_pos += bytes_written;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  int16_t file_name_bytes = strlen(file_name);  // change this
...
static int WriteDocIDToPositionListFn(FILE* f, IndexFileOffset_t offset,
  ...
  Verify333(it != nullptr);
  IndexFileOffset_t position_pos = offset + sizeof(DocIDElementHeader);
  for (int i = 0; i < num_positions; i++) {
    // STEP 13.
    // Get the next position from the list.
    uint64_t next_position;
    LLIterator_Get(it, reinterpret_cast<LLPayload_t*>(&next_position));
    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    position.position = static_cast<DocPositionOffset_t>(next_position);
    position.ToDiskFormat();
    if (fseek(f, position_pos, SEEK_SET) != 0) {
      return kFailedWrite;
    }
    ...
    position_pos += sizeof(DocIDElementPosition);
  ...
  return position_pos - offset;  // you may want to change this
}
...
static int WriteWordToPostingsFn(FILE* f, IndexFileOffset_t offset,
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(WordPostingsHeader) + word_bytes + ht_bytes;  // change this
}
}  // namespace hw3
